...
export async function getAllPlans(supabase: SupabaseClient<Database>): Promise<Plan[]> {
  const { data } = await supabase
    ...
    .select(
      "id, slug, name, price, classes_per_month, free_express_per_month, reschedules_per_month, express_discount, rollover_classes, sort_order"
    )
    .eq("is_active", true)
    .order("sort_order");
  ...
  return (data ?? []).map((p) => ({
    ...
    classes_per_month: p.classes_per_month ?? 0,
    ...
    rollover_classes: p.rollover_classes ?? 0,
...
export async function getActivePlan(
  ...
  return {
    ...
    classes_per_month: row.classes_per_month ?? 0,
    ...
    rollover_classes: row.rollover_classes ?? 0,
    remaining_classes: row.remaining_classes ?? 0,
