...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  NO_TEACHER_AVAILABLE: "No encontramos profesores disponibles para esa materia por ahora.",
};
...
export async function POST(req: Request) {
  ...
  if (!body?.subjectId || !body?.scheduledAt || !body?.durationMin) {
  ...
  const { data, error } = await (supabase as any).rpc("create_auto_assigned_booking", {
  ...
  notifyTeacherOfNewBooking(supabase, user.id, data as string).catch((e) => {
    console.error("[bookings/auto-assign] No se pudo notificar al profesor por email:", e);
  });
